# Sober AI Roadmap

## Vision

Deliver a privacy-safe, tenant-isolated recovery accountability platform that replaces paper attendance and manual follow-up with verifiable digital workflows. Prioritize iOS-first field workflows while preserving supervisor controls, auditability, and secure defaults.

## Slice Status

- Slice A: DONE
- Slice B: DONE
- Slice C: DONE
- Slice D: IN PROGRESS

## Release v0.9.4 (Meetings Liftoff)

- Geo distance reliability: normalized meeting coordinates, shared haversine utility, and cached location service for on-device distance rendering in meeting + home-group lists.
- Permission UX: explicit `While Using App` and `Always (recommended)` location actions with in-app denied-state guidance and settings deep links.
- Attend + signature flow: in-progress meetings now actionable with `Happening now` treatment, strict signature-before-end behavior, and one-time chair-signature prompt per attendance instance.
- Export hardening: replaced brittle export path with multi-page AA/NA Attendance Slip PDF generation and share flow.
- Calendar integration: attend flow now prompts `Add to calendar`, creates meeting event metadata, and stores `calendarEventId` on attendance records to prevent duplicates.

## Release v0.9.5 (iOS Device Location Restore)

- iOS native permission config hardened for dev/client and production parity (`NSLocationWhenInUseUsageDescription`, `NSLocationAlwaysAndWhenInUseUsageDescription`, `NSLocationAlwaysUsageDescription`, and `UIBackgroundModes`).
- Device location fetch made deterministic with explicit service checks (`hasServicesEnabledAsync`), robust foreground permission handling, timeout behavior, and last-known location fallback.
- Meetings refresh now re-checks permission and current location on app focus/return, restoring distance rendering after Settings changes without reinstall.
- Location-state UX now differentiates denied permission vs. disabled Location Services vs. missing meeting coordinates, preventing false generic `Location unavailable` states.
- Sponsor time picker crash hardening: keep Android on the native dialog, replace iOS with a sheet-based selector, and add a defensive fallback if the native Android picker cannot open.
- Calendar automation now uses native calendar add/edit flows instead of ICS export, auto-syncs sponsor calls and attended meetings once permission is available, and advances queued meeting attendance through geofence-aware start/end automation with signature-aware completion states.
- Meetings auth + discovery hardening: mobile now reads runtime Expo config for the live API/auth path, avoids malformed simulator headers, uses device-local timezone keys for meetings/day windows, builds a deduped 7-day home-group selector from area meetings, and adds an `Online Meetings Now` CTA from the dashboard tile.
- Recovery gauges enhancement: setup step 2 now supports Marijuana and Kratom, the dashboard now splits Mental and Physical recovery into separate educational gauges, and each gauge opens a substance-switchable week-by-week guide with supportive trend language.
- Upcoming meetings tile refinement: the live dashboard now uses the shared compact meetings tile, defaults to the next two area meetings, exposes an expand/collapse control for the fuller list, and keeps the `Online Meetings Now` quick action visible beside the attendance log.
- Home group birthdays: users can now attach a stable home-group identity to birthday participation, opt into first-name-only sobriety birthday recognition from setup or meeting detail, edit the setting later, and see a once-per-day in-app announcement when someone else in the same home group is celebrating.

## Slice A (DONE)

- User features: meetings directory, attendance check-in/check-out, verifier signature, supervisor attendance list.
- API endpoints: `POST /v1/meetings`, `GET /v1/meetings`, `POST /v1/attendance/check-in`, `POST /v1/attendance/check-out`, `POST /v1/attendance/:attendanceId/sign`, `GET /v1/supervisor/attendance`.
- Data tables: `meetings`, `attendance`, `verifier_signatures`.

## Slice B (DONE)

- User features: exclusion zones, assigned zone rules, incident reporting, supervisor incident list, notification event queueing.
- API endpoints: `POST /v1/zones`, `GET /v1/zones`, `POST /v1/users/:userId/zones`, `POST /v1/incidents/report`, `GET /v1/supervisor/incidents`.
- Data tables: `exclusion_zones`, `user_zone_rules`, `incidents`, `notification_events`.

## Slice C (DONE)

- User features: supervision mode pings, live supervisor map feed, user supervision controls, compliance events.
- API endpoints: `POST /v1/location/ping`, `GET /v1/supervisor/live`, `GET /v1/me/zones`, `PUT /v1/users/:userId/supervision`.
- Data tables: `last_known_locations`, `compliance_events`, `users.supervision_enabled`, `users.supervision_end_date`.

## Slice D (IN PROGRESS)

- User features: sponsor contact config, sponsor enable toggle + reminder toggle restructuring, single-source sponsor status hygiene, simulator-safe call-now handling, reliable location enable messaging, and meeting drive reminder planning.
- Sober house foundation starts with persisted data models, a settings system of record, configurable rule storage, and field-level audit logging before any enforcement engines ship.
- Sober house resident layer now adds onboarding/editable resident housing profiles, resident requirement branching, house assignment, inheritance-ready defaults, and consent acknowledgment capture with stored signature references.
- Sober house compliance engine v1 now evaluates curfew, near-miss windows, chores, work/job-search status, and weekly meeting quotas using resident overrides, house rules, current location, persisted resident inputs, and the existing attendance source of truth.
- Sober house intervention layer now persists deduped violations, corrective actions, evidence linkage, and manager resolution workflow on top of compliance outputs with audit-safe history.
- Structured internal chat foundation now persists direct sober-house manager/resident threads, structured message types, violation-linked thread reuse, unread/read receipts, acknowledgment-required notices, and auditable message history.
- Sober house monthly reporting now stores immutable resident and house report snapshots with monthly KPI summaries, wins/streak summaries, report history, and in-app detail views.
- Sober house report output workflow now adds review states, locked final manager summaries, version-safe regeneration, resident/house PDF export from stored snapshots, export history, and distribution-ready metadata without delivery automation.
- Sober house dashboard blending Part A now adds resident-only chore, weekly meeting, and next-obligation dashboard tiles backed by reusable summary selectors and house-config-aware visibility on the existing recovery dashboard.
- Sober house dashboard blending Part B now adds conditional one-on-one setup in the resident flow, persisted one-on-one scheduling fields, calendar event mapping, centralized reminder scheduling, and next-up dashboard integration without introducing a separate scheduling engine.
- Recovery UX v2: setup wizard flow, dashboard-first home experience, purple liquid-glass cards, and local recovery analytics (days sober, 90-day progress, sponsor adherence).
- Recovery milestone UX: after the initial 90-day meeting window, the dashboard now switches to the next sobriety benchmark coin with days-remaining tracking, a native purple/gold AA-style spinning medallion, a roadmap view of achieved/upcoming benchmarks, and a once-per-milestone celebration splash on exact sobriety birthdays.
- Meetings dashboard balance: the Meetings Attended tile now includes a secondary consistency KPI showing the current streak of calendar weeks with 3 or more meetings attended.
- Physical Recovery feed: the dashboard tile now opens a personalized body-and-brain recovery timeline driven by sobriety date and selected substances, with blended current-stage guidance for alcohol, opioids, and meth/stimulants plus recovery-profile CTA fallback when the user has not personalized it yet.
- Setup wizard service commitments: recurring recovery obligations now support weekly and monthly-ordinal local capture plus calendar series sync/reminder timing on the hardened iOS-safe calendar path.
- Step 7 scheduling polish: recurring service commitments now use 12-hour mobile-native time entry, minute-based arrival/stay offsets, and an export-inclusion flag so selected commitments can appear on AA/NA attendance slip output.
- Recovery-first routed onboarding: the app now starts with a track-selection splash, keeps Recovery mandatory for participant flows, routes sober-house residents into a resident wizard, routes court/program participants into a court selector plus participant wizard, and removes court setup from the end of the Recovery Wizard.
- Protected-role onboarding hardening: selecting an organization or supervision-style path no longer grants protected access locally; hierarchy/configuration routes now require authenticated backend role verification and participant users are limited to participant-facing flows.
- Role-protected hierarchy/configuration access: sober-house and court/admin entry points now use shared access guards so resident and participant users cannot reach hierarchy, role management, or configuration screens through navigation or local onboarding state alone.
- Backend-role-controlled admin access: protected sober-house and court/probation authority now resolves from Postgres-backed scoped role grants exposed through `/v1/me/access-context`, with manual grants supported in Postgres before a dedicated admin panel ships.
- Participant compliance backend foundation: Postgres-backed participant profiles, obligations, compliance events, violations, and proof-verification fields now provide shared operational truth for mobile participant flows and future org/court dashboard filtering without relying on local-only state.
- Sober house admin setup hub: sober-house settings now separate setup modules from operations, with dedicated organization, house groups, houses, rules, and managers flows plus focused house drilldowns and house-violation review from the owner dashboard.
- Sober house settings IA refresh: the sober-house hub now routes each setup/operations tile to the correct module, organization setup embeds editable default rules, house editors expose rule source plus inherited-vs-override summaries, house groups read as reusable ruleset templates, and resident "View sober-house profile" now opens a resident-safe read-only requirements screen instead of the admin setup flow.
- Sober house resident accountability blend: admins can now define scoped recurring house meeting schedules, resident sober-house KPI tiles resolve from effective org/group/house rules, sponsor/meeting/house-meeting progress is no longer hard-coded, and chore proof plus chore completion now feed reporting-ready proof references and participant compliance sync.
- Sober house resident completion and accountability flow: resident setup completion now resolves from required sober-house fields instead of static profile existence, employed residents persist work-location geofence context from setup, unemployed residents surface rule-driven weekly job-application KPIs, and the resident checklist now carries proof-aware chore/application actions that align with effective house rules.
- Sober house resident routine flow: the resident dashboard now exposes a Sober House Routine tile with real percent/open/overdue state, inherited house requirements render as a locked rule-driven checklist, and chore/job-application proof capture persists multi-photo evidence records tied to the resident, task instance, house, and organization for reporting/export follow-through.
- Sober-house chore proof completion polish: resident chore actions now map to rule-driven proof modes, photo and photo-plus-manager-confirmation chores persist multi-photo evidence plus pending manager-confirmation state, and Requirement Status now distinguishes missing proof from awaiting manager review.
- Participant-track lifecycle refactor: recovery now persists as the base profile while sober-housing resident and court participant tracks can be added, resumed, or ended independently from Settings, protected admin roles remain backend-authorized only, and deterministic seeded QA identities now cover recovery, resident, organization, platform, and court states.
- Sober-house admin protected-session cleanup: legacy in-screen unlock/passcode walls are removed from organization setup editors, backend role authorization stays authoritative, and authorized org admins now establish one protected session before navigating Organization, Houses, House Groups, Rules, and Managers.
- Sober-house organization default-rules mobile redesign: organization defaults now render as grouped mobile rule cards with toggle-driven disclosure, clearer proof/verification separation, a calmer full-editor affordance, and a cleaner save area without changing the underlying rule engine.
- Sober-house stabilization pass: seeded QA users now synthesize deterministic protected access when backend access-context fetches are unavailable, Settings exposes a direct sober-house admin re-entry surface for authorized org users, resident rule views now show clearer effective org/group/house inheritance, and setup wizards now support explicit save-and-exit instead of trapping users in repeated back navigation.
- Sober-house operator reporting layer: the owner dashboard now derives centralized org, house, and resident compliance intelligence from memberships, chores, house meetings, work/job-search records, violations, corrective actions, and current report snapshots, with drillable reporting views and deterministic status bands for operator scanning.
- Sober-house explicit completion-event layer: one-on-ones, sponsor calls, and house-meeting adherence now persist as first-class sober-house records with durable completion/missed state, proof-awareness, and operator-side logging actions so reporting and future exports can rely on explicit completion truth instead of partial inference.
- Sober-house exports and scheduled summaries: the Reports console now generates filtered operator PDF/CSV outputs for resident, house, organization, violations, and missing-proof views, persists recent export metadata plus in-app daily/weekly summary snapshots, and reuses the centralized operator-reporting model instead of duplicating report logic.
- Sober-house operator web dashboard v1: the dashboard app now acts as a desktop control plane with org → house → resident hierarchy, searchable resident lookup, staff and house drilldowns, rule-source visibility across org defaults / house overrides / resident exceptions, and direct access to the existing reporting, export, and scheduled-summary systems without duplicating sober-house business logic.
- Sober-house escalation and enforcement v1: sober-house data now includes durable escalation records with operator history, deterministic threshold-based recommendations over chores/curfew/house meetings/one-on-ones/sponsor calls/work/job-search/proof/violations, and a web action queue with acknowledge, assign, note, resolve, escalate, incident-link, and resident intervention timeline support.
- Sober-house live operator control plane: the dashboard now authenticates against the live sober-house control-plane API, hydrates org-scoped houses/residents/staff/rules/reports/summaries/enforcement from persisted organization data instead of seeded demo state, persists operator queue updates back through the API, and shows explicit unauthenticated / forbidden / empty-org states for real operator use.
- Sober-house role workflow reliability: approved housing admins can now bootstrap exactly one organization from mobile, platform owners retain cross-org visibility, resident setup now binds the entered resident name plus selected org/house/group into the shared backend participant profile, and seeded QA identities cover `Kacy Housing Admin`, `Demo`, `Jason SaaS Admin`, and resident assignment validation without regressing Alpine demo flows.
- Sober-house admin bootstrap persistence: first-time housing-admin setup now treats the wizard fields as the authoritative source for org bootstrap, writes the selected QA/runtime identity instead of the base dev user, updates the linked operator profile on the backend during first bootstrap, and rehydrates the dashboard from the persisted control-plane snapshot so org/contact values survive scenario switching, restart, and re-login.
- Sober-house proof review and approval v1: sober-house data now includes durable proof-review records with history, the web control plane now exposes a proof-review queue with approve/reject/pending/follow-up/note actions plus resident/house/org proof visibility, and enforcement/report exports/summaries now distinguish missing proof from submitted-pending and rejected proof pressure.
- iOS calendar launch-crash hardening: Expo Calendar device builds now include the required reminders plist descriptions alongside calendar strings, and the app refuses to touch native calendar APIs if that config is missing.
- Home Group birthdays: setup and settings flows now use clearer opt-in copy, explicit name labels, same-home-group-only recognition messaging, and safe partial-save handling without setup-time birthday lookups or raw backend errors in the UI.
- Tools (Recovery mode): AM routine checklist, nightly inventory CRUD, local routines analytics, sponsor SMS share, and routine PDF exports.
- Dashboard: evolve the existing direct-message foundation into broader recovery/service/probation chat flows after sober-house direct messaging proves out.
- Dashboard: Wisdom tile uses server-driven daily rotation (`/api/wisdom/daily`) with one quote per day, pronoun normalization (`I/you -> we`), plus local cache and deterministic fallback.
- Mobile dev correctness: API URL alignment (`localhost:3031`), no hardcoded SF map origin defaults, nearby-meetings request diagnostics, and map search-origin visibility.
- Build readiness: Development Build workflow implemented (EAS `development` / `preview` / `production` profiles, environment-driven bundle identifiers, stable deep-link scheme, and dev-client runbook in `docs/DEV_BUILD.md`).
- Deterministic mobile API environment selection: Render is now the explicit default backend for normal use, localhost requires an intentional development override, runtime localhost inference is removed, and Diagnostics now shows the active API base URL, backend selection, auth mode, identity, tenant, and loaded backend roles.
- Dev/QA role harness: mobile Settings now includes a dev-only one-tap scenario switcher for new housing admin, existing housing admin, resident, platform owner, and unknown-user onboarding QA, with app-owned local-state reset, seeded identity application, clean rehydration, and visible resolved scenario/identity status for repeatable iOS testing.
- Preview pipeline: internal preview build profile + TestFlight-ready store preview profile documented and validated in `docs/PREVIEW_BUILD.md`.
- API dev tooling: DEV-auth-gated meeting ingest trigger (`POST /v1/dev/meetings/refresh`) with cooldown guard for local workflows.
- API endpoints: `GET /v1/me/sponsor`, `PUT /v1/me/sponsor`, `POST /v1/dev/meetings/refresh`.
- Data tables: `sponsor_config`.

## Futures / Backlog

- [ ] Sober house follow-on slices: alert delivery automation, distribution workflows, and dashboards on top of the settings foundation, compliance/intervention layers, chat foundation, and monthly report exports.
- [ ] Auto-request location permission on first launch; if denied, show Settings deep-link CTA.
- [ ] Import AA/NA meetings feeds (Meeting Guide spec) and return meetings within 20-mile radius when location is available.
- [ ] Meeting Guide distributed feed ingest + tenant-scoped `/v1/meetings/nearby` search (20-mile default) with map/list toggle UX.
- [ ] Dashboard + Setup Wizard + upcoming-meetings logic hardening across timezones and tenant policy controls.
- [ ] Tools: AM routine + Nightly inventory + insights (local-first storage, optional API sync later).
- [ ] Expand the structured chat foundation to provider-backed invites, push notifications, deep links, group chat, and moderation controls.
