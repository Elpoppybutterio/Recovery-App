...
- Recovery setup UX: sponsor call time now uses a native-feeling time picker in both the Recovery Wizard and sponsor settings, replacing the old hour/minute stepper without changing downstream reminder scheduling.
...
- Meetings dashboard UX: the Home upcoming-meetings tile now defaults to the next meeting with an expand-to-five toggle, uses a denser compact card layout, and adds an `Online meetings now` action that prefers a live meeting URL and falls back to the official Online Intergroup directory.
- Physical Recovery feed: the dashboard tile now opens a personalized body-and-brain recovery timeline driven by sobriety date and selected substances, with blended current-stage guidance for alcohol, opioids, meth/stimulants, marijuana, and kratom, plus an in-place `Change substances` flow that returns directly to the recovery screen after saving; the tile now shows paired mental-repair and physical-repair half-arc gauges, and each lens opens a weekly recovery panel focused on either brain chemistry/thought-emotion changes or body-repair/physical presentation.
- Sober-house settings access: owner/operator and resident sober-house settings can now unlock with Face ID first and a user-configured 7-digit fallback passcode, with in-app setup controls and audit entries for passcode/security changes.
...
- Morning routine hardening: sponsor check-in completion now requires the actual daily AM-text action, unsupported play pills are removed from read-only prayers/readings, and meditation defaults to the Spotify show fallback while preserving editable custom links.
