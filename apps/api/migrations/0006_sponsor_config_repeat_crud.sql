...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'sponsor_config_repeat_shape_check'
  ) THEN
    ALTER TABLE sponsor_config
      ADD CONSTRAINT sponsor_config_repeat_shape_check
      CHECK (
        (
          repeat_unit = 'MONTHLY'
          AND repeat_interval = 1
          AND cardinality(repeat_days) = 0
        )
        OR (
          repeat_unit = 'WEEKLY'
          AND repeat_interval IN (1, 2)
          AND cardinality(repeat_days) >= 1
        )
      );
  END IF;
END
